...
function debuggerAllowsNetworkOnlyFrontSubmit(task = {}) {
  if (String(task.repeatSlotMode || "") === "repeat_first_reference") return true;
  const mode = String(task.mode || "");
  const repeatCount = Math.max(1, Number(task.repeatCount || task.expectedVideos || 1) || 1);
  if ((mode === "image-to-video" || mode === "start-end-image-to-video") && repeatCount > 1) return true;
  const promptCount = Math.max(1, Number(task.jobPromptCount || task.promptCount || 1) || 1);
  if (["image-to-video", "start-end-image-to-video", "ingredients-to-video"].includes(mode) && promptCount > 1) return true;
...
async function debuggerFrontSubmitSnapshot(target, task = {}, options = {}) {
  ...
  return debuggerEvaluate(target, `((expectedPrompt, expectedMode, expectedMediaIds, baselineProjectCardIds, baselineGenerationProjectCardCount) => {
	    const visible = (element) => {
	      if (!element) return false;
	      const style = getComputedStyle(element);
      const rect = element.getBoundingClientRect();
    ...
    const createButtons = Array.from(document.querySelectorAll("button,[role='button']"))
      ...
      .map((node) => ({
	    ...
	    const projectCards = Array.from(document.querySelectorAll("[data-tile-id]"))
	      ...
	      .map((node) => {
	        ...
	        const progressLike = /\\b([1-9]\\d?|100)%\\b|progress_activity|generating|loading/i.test(text);
	    ...
	    const generationProjectCards = isVideoMode
	      ? newProjectCards.filter((card) => card.videoLike || card.progressLike)
	      : newProjectCards;
	    const totalGenerationProjectCards = isVideoMode
	      ? projectCards.filter((card) => card.videoLike || card.progressLike)
	      : projectCards;
	  ...
	  })(${promptJson}, ${modeJson}, ${mediaIdsJson}, ${baselineProjectCardIdsJson}, ${baselineGenerationProjectCardCountJson})`).catch((error) => ({ ok: false, error: String(error?.message || error) }));
}
...
async function waitForFrontSubmitTransition(target, task = {}, timeoutMs = 7000, options = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    if (last && last.promptStillVisible === false && !requireStrongProof) {
  ...
  return {
    ok: false,
    reason: last?.promptStillVisible === false && requireStrongProof
  ...
  return {
    async submitTask(tabId, task = {}, meta = {}) {
      ...
      try {
        ...
        const prepareForDebugger = async (stage = "prep", metaPatch = {}, timeoutMs = 120000) => {
        ...
        const prepared = await prepareForDebugger("prep_initial");
        ...
        if (!finalPrepared?.ok && /SETTINGS|VIDEO|MODEL|COMPOSER_UPLOAD_NOT_SETTLED|videoApi|videoModelKey/i.test(String(finalPrepared?.error || ""))) {
        ...
        if (debuggerShouldRecommitPromptBeforeFinalClick(task)) {
          const finalCommitPrep = await sendPageCommand({
            action: "domCommitPromptForDebugger",
            task,
          ...
          const finalTyped = finalCommitPrep?.result?.result || finalCommitPrep?.result || finalCommitPrep;
          if (finalTyped?.ok) {
        ...
        if (enterSubmit) {
          ...
          await sleep(450);
          const afterEnterSnapshot = await debuggerFrontSubmitSnapshot(target, task, { baselineProjectCardIds, baselineGenerationProjectCardCount });
        ...
        let frontTransition = debuggerRequiresFrontSubmitTransition(task)
          ? await waitForFrontSubmitTransition(target, task, 7000, { baselineProjectCardIds, baselineGenerationProjectCardCount })
        ...
        const pageCaptureProvedNoRequest = noRequestErrorPattern.test(pageCaptureError);
        const pageCaptureObservedRequest = !pageCaptureProvedNoRequest && (Number(pageResponse?.status || 0) > 0
        ...
        const frontSubmitAlreadyMoved = frontSubmitTransitionHasStrongProof(frontTransition, task);
        const frontSubmitProvesGeneration = frontSubmitTransitionProvesGeneration(frontTransition, task);
        const frontSubmitHasActiveProgressProof = frontSubmitTransitionHasActiveProgressProof(frontTransition);
        const shouldRetryNoRequest = !frontSubmitAlreadyMoved || !frontSubmitProvesGeneration || !frontSubmitHasActiveProgressProof;
        if (!mediaIds.length && !pageCaptureObservedRequest && shouldRetryNoRequest && noRequestErrorPattern.test(String(response?.error || pageResponse?.error || ""))) {
          ...
          const retryDetailClose = await debuggerCloseImageDetailIfOpen(target).catch((error) => ({ ok: false, error: String(error?.message || error) }));
          ...
          let retryCanClickCreate = Boolean(retryArmed?.ok && hitLooksLikeCreateButton(retryHit));
          if (retryArmed?.ok && !retryCanClickCreate) {
            ...
            await debuggerPressKey(target, "Escape", "Escape", 27, { holdMs: 25 }).catch(() => {});
            await sleep(550);
            const recoveredDetailClose = await debuggerCloseImageDetailIfOpen(target).catch((error) => ({ ok: false, error: String(error?.message || error) }));
            if (recoveredDetailClose?.closed) await sleep(900);
            const recoveredCreate = await debuggerStableCreateButtonPoint(target, 5200, { minWaitMs: 900 });
            ...
            if (hitLooksLikeCreateButton(recoveredHit)) {
        ...
        if (debuggerRequiresFrontSubmitTransition(task) && !frontSubmitTransitionHasStrongProof(frontTransition, task) && mediaIds.length) {
          const refreshedFrontTransition = await waitForFrontSubmitTransition(target, task, debuggerPostResponseFrontProofWaitMs(task), { mediaIds, baselineProjectCardIds, baselineGenerationProjectCardCount, requireStrongProof: true });
        ...
        if (frontSubmitProofMissing
          ...
          const reloadFrontTransition = reloadResult.ok
            ? await waitForFrontSubmitTransition(target, task, 24000, { mediaIds, baselineProjectCardIds, baselineGenerationProjectCardCount, requireStrongProof: true })
        ...
        const frontSubmitNetworkOnlyAccepted = frontSubmitProofMissing
          && allowNetworkOnlyFrontSubmit
          && responseStatusForFrontProof >= 200
          && responseStatusForFrontProof < 300
          && (String(task.mode || "") === "text-to-image" || mediaIds.length >= expectedCount);
        if (frontSubmitProofMissing && mediaIds.length) {
          recordTrace(task, frontSubmitNetworkOnlyAccepted
            ? "front_submit_transition_network_only_accepted"
            : "front_submit_transition_network_only_rejected", {
            reason: frontTransition.reason || "front_transition_missing_but_dom_response_confirmed",
            allowNetworkOnlyFrontSubmit,
            mediaIdCount: mediaIds.length,
            mediaIds,
